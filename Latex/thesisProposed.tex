...
it scales linearly with each additional WES.
...
\begin{table}[h]
	...
	\begin{tabular} { lrr }
		...
		MAC headers							& 28	& 224 \\
		MAC body							& \textit{20} & \textit{160}\\
		\textbf{= tx time data}				& 		& \textbf{626} \\
		SIFS								& -		& 10 \\
		PHY header: PLCP preamble			& 18	& 144 \\
		PHY header: PLCP header				& 6		& 48 \\
...
so the average should take $\frac{CW_{min}}{2}$ slots. 
Given a slottime of 20$\mu$s the average backoff takes 160$\mu$s.

In order to avoid unnecessary load on the radio channel, Art-Net transmit only the changes.
In the worst case, however, this affects all WESs.
...
The idea of the unicast is, that a transmission to each device is very fast, because the transmitted payload is smaller.
