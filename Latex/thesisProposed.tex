...
\begin{figure}[h]
	\centering
	\includegraphics[scale=0.5]{/home/walther/Documents/bachelor/Plot2/Graphs/bc_analytic.pdf}
...
\begin{figure}[h]
	\centering
	\includegraphics[scale=0.5]{/home/walther/Documents/bachelor/Plot2/Graphs/bc_uc_transmissiontime_analytic.pdf}
...
\begin{figure}[h]
	\centering
	\includegraphics[scale=0.5]{/home/walther/Documents/bachelor/Plot2/Graphs/rr_transmission_time.pdf}
...
In this chapter a short hands-on is given on how to flash the microcontrollers for the testbed
and which steps are necessary to get ESP-NOW running on theses chips.
It also shows how the measurement data was collected,
how the test setup was distributed from the PC to all the chips and how the data was subsequently returned to the PC.
...
Later, the individual MAC addresses of the WESs must be entered.
...
\end{lstlisting}
\label{lst:callback}
Eine Herausforderung bei der Programmierung des Controllers und der WESs, 
ist die zustandbasierte Herangehensweise und die Tatsache, dass die WESs nicht miteinander kommunizieren konnten.
Es gab demnach zwei Arten von State-Machines, eine für den Controllern und die andere für die WESs.

Das Testsetup besteht aus einem PC, der die Daten an den Controller weiter gibt, dieser verteilt die Daten weiter via ESP-NOW an die WESs.
Für das Testbed wurde mit sieben WESs gearbeitet.
\TODO{Bild vom Testsetup}
...
Vom PC aus wird das Experiment gestartet, mittels Python Skript wird dann ein JSON über \ac{UART} an den Controller übermittelt.
Im JSON stehen alle Parameter die für die Durchführung eines Tests benötigt sind.\href{tab:json}
Zum Zeitpunkt der übertragung des JSON Strings sollte der Controller im idle sein, damit die Pakete korrekt eingelesen werden.
\TODO{Tabelle des JSON mit Erklärung}
...
\begin{figure}
	\includegraphics[scale=0.8]{figures/sequence_diagram_drive.pdf}
