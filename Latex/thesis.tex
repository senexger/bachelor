...
It is a conectionless protocol, so the \ac{WES} are in Ad-Hoc mode insted of \ac{STA} and is compatible with the ESP8266, ESP32 and ESP32s. 
Through the hardware limitation it can just be used on the 2.4 GHz frequncy band.
ESP-NOW allows up to 20 ESPs for pairing or up to 10 if they they use encryption.
Espressif promises throuput of up to 30MBit/s UDO over the air and a possible range of up to 1km.
However Pasic et al. measured the range of the unmodified onboard antenna of the ESP32. 
They got a \textbf{stable commication up to 190m in open field}.\todo{cite Roberto}\\
...
it also has a much less overhead, which results in shorter airtime, less disturbances and also less power consumption through the antenna (which is not relevant for this thesis).
There is no TCP/IP header to be transmited. 
For very small payloads, this offset can be dispropotional.

Unfortunately, the documentation is quite limited. \\

ESP-NOW uses a vendor-specific action frame to transmit ESP-NOW data.
The default ESP-NOW bit rate is 1 Mbps. 
The format of the vendor-specific action frame is \todo{ESP documentation website}
...
\begin{itemize}
	...
	\item MAC Header: As ESP-NOW is connectionless, the MAC header is a little different from that of standard frames.
	\item Category Code: The Category Code field is set to the value(127) indicating the vendor-specific category.
	\item Organization Identifier: The Organization Identifier contains a unique identifier (0x18fe34), which is the first three bytes of MAC address applied by Espressif.
	\item Random Value: The Random Value filed is used to prevents relay attacks.
	\item Vendor Specific Content: The Vendor Specific Content contains vendor-specific fields as follows:
	\item Frame Check Sequence: Used for error correction in layer 2
\end{itemize}

Inside of the ESP-NOW frame is the Vendor Specific Content visualized in \ref{fig:esp_now_vendor_format}. 
...
\begin{table}[h]
	...
	\end{tblr}
	\begin{tabularx}{\linewidth}{ X X X X X X }
		\makecell{\footnotesize{1}} & \makecell{\footnotesize{1}} & \makecell{\footnotesize{3}} & \makecell{\footnotesize{1}} & \makecell{\footnotesize{4}} & \makecell{\footnotesize{7 $\sim$ 255}} \\
	\end{tabularx}
...
\begin{itemize}
	\setlength\itemsep{-0.5em}
	\item Element ID: The Element ID field is set to the value (221), indicating the vendor-specific element.
	\item Length: The length is the total length of Organization Identifier, Type, Version and Body.
	\item Organization Identifier: The Organization Identifier contains a unique identifier(0x18fe34), which is the first three bytes of MAC address applied by Espressif.
	\item Type: The Type field is set to the value (4) indicating ESP-NOW.
	\item Version: The Version field is set to the version of ESP-NOW.
	\item Body: The Body contains the ESP-NOW data.
	\todo{this is cited from espressif manual!!}
\end{itemize}

Where the Vendor Specific Content is formatted like this. 
Which can be Lorem Ipsum Excalibur.
