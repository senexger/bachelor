...
\acrodef{LLC}{Locig Linc Control}
...
\subsection{Media Access Control Layer}

The MAC is the second lowest layer of the \ac{OSI} Model \ref{tab:OSI} and includes network protocols that regulate how multiple computers share the physical transmission medium they use. 
...
The DMX controller is in the begin of each chain, just having an output.
The receiving devices, in DMX terms called fixtures, are chained from output to input. 
A terminator is specified in the DMX specification that is to be connected to the final output.
The hole chain is called DMX-Universe and can contain a set of 512 channel. 
If there is a need of more channels one needs more DMX universes.
