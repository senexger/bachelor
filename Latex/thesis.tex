...
In ESP-NOW, application data is encapsulated in a vendor-specific action frame and then transmitted from one Wi-Fi device to another without connection. 
...
\TODO{cite or find own words}

\begin{table}[h]
	\centering
	\begin{tabularx}{\linewidth}{|X|X|X|X|X|X|}
		\hline
		\makecell{MAC\\Header}& \makecell{Category\\Code} & \makecell{Org.} & \makecell{Random\\Values} & \makecell{Vendor SC} & \makecell{FCS} \\
		\hline
		\makecell{24 bytes} & \makecell{1 byte} & \makecell{3 byte} & \makecell{4 byte} & \makecell{7 $\sim$ 255\\bytes} & \makecell{4 bytes} \\
		\hline
	\end{tabularx}
	\begin{itemize}
		...
		\item Frame Check Sequence: The Vendor Specific Content contains vendor-specific fields as follows:
	\end{itemize}
...
\begin{table}[h]
	\begin{tabularx}{\linewidth}{|X|X|X|X|X|X|}
		...
		\hline
		\makecell{1 byte} & \makecell{1 byte} & \makecell{3 byte} & \makecell{1 byte} & \makecell{1 byte} & \makecell{7 $\sim$ 255\\bytes} \\
		\hline
	\end{tabularx}
	\begin{itemize}
		\setlength\itemsep{-0.5em}
