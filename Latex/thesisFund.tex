...
The \ac{DL} Layer is split in two layers. 
\begin{itemize}
	\item \ac{MAC} Layer, which provides control for accessing the transmission medium.
	It's responsible for moving packets from one endsystem to another, across a shared transmission medium.
	\item \ac{LLC} multiplex protocols over the MAC layer while transmitting and to de-multiplex the protocols while receiving.
...
The \ac{MAC} is the second lowest layer of the \ac{OSI} Model \ref{tab:OSI} and includes network protocols that regulate how multiple computers share the physical transmission medium they use. 
...
The bus signal is updated with $\frac{1}{50}$Hz.
...
The endsystems are called fixture because it's most likely a lighting instrument, 
