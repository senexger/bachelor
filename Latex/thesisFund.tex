...
an approach for robust multicast in video transmissions is given by Jérome Lacan et al. in \cite{EvaluationOfErrorControl}.
...
Almost every 802.11 capable Microcontroller unit could be picked for this research.
...
The most recent ESP32 datasheet can be found the documentation page of Espressif.
The chip \ref{fig:esp32} is promoted with several features \cite{ESP32Datasheet}:
\begin{itemize}
	\setlength\itemsep{-0.0em}
...
\emph{The extensive set of features makes it suitable for integration into a wide range of smart environments.}
...
They also claim in the ESP-NOW User Guide \cite{ESPNOWUserGuide}, that \emph{broadcast is not supported}, but it is.
