...
\begin{itemize}
	\setlength\itemsep{-0.0em}
	\item \textbf{CS (Carrier Sense):} jede Station checkt ob das Medium frei ist, bevor sie sendet
	\item \textbf{MA (Multiple Access)}: mehrer Stationen teilen sich ein Medium, dieses kann auch ein ein Kabel sein
	\item \textbf{CD (Collision Detection):} Ein Ablaufplan verhindert, dass zwei Stationen zeigleich ihre Übertragung beginnen
\end{itemize}

Before a station transmitts data it has to check or listen to the medium to avoid collisiion resulting in packet loss.
Only when the medium is free, the station can transmit via the shared medium.
When the medium is already in use t

Collision avoidance tries to avoid collisions before they happen in the first place.
