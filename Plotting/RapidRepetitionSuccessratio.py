...
arraySR = np.genfromtxt(
    "/home/walther/Documents/bachelor/Data/latency/broadcast/broadcast7NodesRR3.csv", delimiter=",", dtype="int")
...
def grouping_plot(array):
    ...
    ax2.set_ylabel('Success Ratio in %', color='red')
    ...
    plt.grid()
    plt.savefig(
        '/home/walther/Documents/bachelor/Graphs/SRvsRR_FINAL.png', dpi=2000)
    plt.show()
...
def decodeEverything(array, rr):
    decodeEverything = np.zeros(
        (SLAVE_COUNT, TEST_REPETITION*SEND_REPETITION), dtype=int)
...
def successRatio_plot(array1, array2, array3):

    std_errors = np.zeros(SLAVE_COUNT)
    means = np.zeros(SLAVE_COUNT)

    # Don't judge me, there is no time for nice code
    for node in range(0,SLAVE_COUNT):
        std_errors[node] = np.std(array1[node,:], ddof=1) / np.sqrt(np.size(np.std(array1[node,:])) / 100)
        means[node] = np.mean(array1[node,:])
    print(std_errors)
    print(means*100)
    x = ["#1", "#2", "#3", "#4", "#5", "#6", "#7"]
    plt.errorbar(x, means*100, yerr=std_errors/1.5, fmt='-o',
                 color='b', markersize=8, linestyle='none', capsize=7)

    # Don't judge me, there is no time for nice code
    for node in range(0,SLAVE_COUNT):
        std_errors[node] = np.std(array2[node,:], ddof=1) / np.sqrt(np.size(np.std(array2[node,:])) / 100)
        means[node] = np.mean(array2[node,:])
    print(std_errors)
    print(means*100)
    plt.errorbar(x, means*100, yerr=std_errors/1.5, fmt='-o',
                 color='r', markersize=8, linestyle='none', capsize=7)

    # Don't judge me, there is no time for nice code
    for node in range(0,SLAVE_COUNT):
        std_errors[node] = np.std(array3[node,:], ddof=1) / np.sqrt(np.size(np.std(array3[node,:])) / 100)
        means[node] = np.mean(array3[node,:])
    print(std_errors)
    print(means*100)
    plt.errorbar(x, means*100, yerr=std_errors/1.5, fmt='-o',
                 color='g', markersize=8, linestyle='none', capsize=7)
    ...
    plt.title('Sucess Ratio for Broadcast with/without Rapid Repition')
    plt.ylabel('Success Ratio in %')
    plt.xlabel('Slave ID')
    plt.grid()
    ax1.legend(['RR=0', 'RR=1', 'RR=2'])
    # fig.patch.set_facecolor('xkcd:white')

    plt.savefig(
        '/home/walther/Documents/bachelor/Graphs/successRatioPerNode_RR2.png', dpi=1000)
    plt.show()

arrayByNodesRR0 = decodeEverything(arraySR, 1)
arrayByNodesRR1 = decodeEverything(arraySR, 2)
arrayByNodesRR2 = decodeEverything(arraySR, 3)
arrayRR = [arrayByNodesRR0, arrayByNodesRR1, arrayByNodesRR2]
